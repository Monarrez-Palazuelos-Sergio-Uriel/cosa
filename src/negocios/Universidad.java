...
public class Universidad {
	...
	* Constructor por defecto que inicializa las listas de estudiantes, productos y préstamos.
		...
		try {
			for (int i = 0; i < prestamos.getTamanio(); i++) {
				...
				if (p != null && p.getEstudiante() != null) {
					...
					// apilar en orden de lectura (suponiendo orden cronológico) -> cima será el último
					pila.apilar(p);
	...
	public boolean prestarProducto(int codigo, int id, Fecha fecha) throws PosicionIlegalException{
		// TODO (Alumno): implementar la lógica de préstamo
		// Debe verificar existencia de estudiante y producto, checar préstamos activos,
		// crear un Prestamo, actualizar listas/pilas/colas y marcar producto como prestado.
		// Por ahora devolvemos false para indicar que no está implementado.
		return false;
	}
	...
	public Estudiante consultarEstudianteTieneProducto(int id) throws PosicionIlegalException{
		// TODO (Alumno): implementar búsqueda del estudiante que tiene prestado el producto
		// Debe revisar la lista `prestamos` y retornar el Estudiante cuyo préstamo esté activo.
		return null;
	}
	...
	public ListaDoble<Producto> consultarProductosDeUnEstudiante(int codigo) throws
	   PosicionIlegalException {
		// TODO (Alumno): implementar la recuperación de productos prestados a un estudiante
		// Debe devolver una ListaDoble<Producto> con los productos que tengan préstamos
		// activos para el código proporcionado.
		return new ListaDoble<Producto>();
	}
	...
	public boolean devolverProducto(int id, Fecha fechaDev) throws PosicionIlegalException{
		// TODO (Alumno): implementar la lógica de devolución
		// Validar fecha, actualizar el préstamo y marcar el producto como disponible.
		return false;
	}
	...
	public boolean eliminarProducto(int id) throws PosicionIlegalException{
		// TODO (Alumno): implementar eliminación de producto
		// Debe remover préstamos asociados, quitar el producto de la lista correspondiente
		// y actualizar el índice invertido.
		return false;
	}
